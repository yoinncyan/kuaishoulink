...
- 删除通用 Profile 管理界面。
- 提供多账号持久化 Profile 注册、选择和切换；每个 Profile 的账号、指纹与缓存相互隔离。
...
- 多账号持久化快手 Profile；每个 Profile 独立 user-data/identity/缓存，浏览器与探针生命周期分离。
...
- 本地 Git 分支：`main`。
- 关键历史提交：`46c9c32`（网络探针/解析器/Web 面板）、`0905447`（登录态全量滚动）、`c587b48`（关闭窗口恢复）、`33758b6`（静默分页异常诊断）。
- 最新专项测试：49 项通过；前端 TypeScript/Vite 生产构建通过；`git diff --check` 通过。
- 本地 Web 服务当前运行在 `127.0.0.1:8080`，已加载多 Profile 管理界面；原 Profile 已无损登记为 `kuaishou / 默认账号`，原 identity ID/指纹种子和快手登录状态保持不变。历史匿名模式检查点仍独立保留。正式登录态任务已完成 22/2,463 个关键词，暂停在第 23 个关键词“vpn路由器”；同一累计主记录为 5,082 条。
...
- 2026-09-10：用户在登录态任务暂停于“vpn下载”探针启动期间直接关闭 CloakBrowser，随后点击继续无响应。现场状态为 `probe_state=running`、`probe.active=false`、`browser_state=stopped`：暂停已停掉探针，但尚未返回的启动协程随后把状态重新写成 running，导致下一次 `/api/task/start` 被“已有探针”拦截。已先把现场状态归一化为 stopped；代码新增探针启动 generation、暂停优先取消、失活探针自动归一化和幂等 stop，旧启动协程不再能覆盖新状态。实测“启动中立即停止”最终保持 stopped；关闭浏览器上下文后再次搜索可自动重启同一登录 Profile，首屏返回 20 条且无风控。
- 2026-09-10：登录态任务在关键词“vpn客户端”的首个滚动分页暂停并报告 `risk-control;2`，但用户在页面未见异常。捕获证据确认不是误报：首屏 `/rest/v/search/feed` 成功返回 20 条、`pcursor=1`；下一页请求携带 `pcursor=1`，HTTP 200 但响应头为 `Intercept-Result: risk-control;2`、Body 为 `result: 2` 且无 `feeds`。快手前端静默保留首屏并显示“没有更多了”，因此肉眼看不到错误；程序暂停是为了避免把 20 条误判为完整结果。用户随后点击继续，同一关键词重新完整采集成功，得到 16 个分页响应、293 条唯一视频并到达 `pcursor=no_more`。现已增强检查点和 Web 错误卡，显示失败阶段、已保存链接、请求游标、HTTP 状态、响应 result 和说明“页面可能看起来正常”；解析器同时保留最后一次失败搜索的 result/游标诊断。
- 2026-09-10：用户确认最终会把整套程序部署到服务器，但仍需在异常、验证码或页面点击问题时人工操作 CloakBrowser。Docker 架构因此确定为“持续有头浏览器 + Xvfb 固定虚拟屏幕 + noVNC/KasmVNC 按需远程接管”，而不是日常 headless、排障时重启切换模式。关闭远程画面只断开观看连接，不关闭浏览器进程；Web 控制台需提供“暂停并接管”“释放并继续”、实时截图/错误信息和嵌入式远程画面，自动化与人工输入互斥。Profile、身份、任务检查点、累计链接和输出均挂载 Volume 持久化，远程桌面入口通过同一 Web 鉴权与反向代理保护，不裸露 VNC/CDP 端口。
- 2026-09-10：用户将原“单一持久化 Profile”决策改为多账号、多 Profile。实现采用 `runtime/profiles/registry.json` 注册表；原 `runtime/profiles/kuaishou` 无损登记为 `kuaishou / 默认账号`，原 identity 复制到该目录内的 `.identity.json`，Profile 目录和登录数据不移动。新 Profile 使用随机 `profile-*` ID 和完全独立目录、`.identity.json`、指纹种子、Cookie/LocalStorage/IndexedDB/HTTP/代码/GPU 缓存及 identity 专属进程临时/XDG 缓存；浏览器二进制缓存继续只读共享。Web 新增 Profile 选择、新建并切换、重命名/独立代理、删除非活动 Profile 的 UI；切换前关闭当前浏览器进程，再加载所选 Profile。相同采集模式在不同 Profile 下选择不同运行目录和关键词检查点，所有 Profile 的视频仍幂等合并到同一累计主记录。深度重置只作用于当前 Profile，不清理其他 Profile。
- 2026-09-10：多 Profile 真实迁移/隔离验证完成。创建临时 Profile 后得到独立目录、独立 identity ID 和独立 63 位指纹种子，登录状态为 false；切回 `kuaishou` 后恢复原 identity ID/种子且 `checkLoginQuery=true`。删除临时 Profile 后其目录和专属临时缓存被清理，默认 Profile、登录状态、任务检查点与累计链接未改变。
...
- 2026-09-10 第二阶段增量检查点：累计数据源已扩至 6,299 条；已完成 2,005 条，其中 2,004 条取得整数评论数、1 条总数不可用，严格大于 50 的视频为 1,523 条，不大于 50 的视频为 481 条，待处理 4,294 条。用户暂停评论任务后，已按当前 Markdown 快照单独更新 `快手视频评论数大于50_最终结果.tsv`；TSV 保留 10 个字段和 1,523 条唯一链接，未改动 XLSX。
- 2026-09-10：按用户要求对上述 1,523 条 TSV 逐条复核并另存 `快手视频评论数大于50_逐条审核提纯版.tsv`。剔除 1,007 条原标记“弱相关”、51 条法院/公安/警察/反诈/法律处罚等警告科普、35 条“翻墙/梯子/机场/虚拟网络”等字面误命中，以及 343 条标题语义与 VPN、境外网络、IP 切换/专线或网络加速无直接关系的内容；最终保留 87 条唯一视频，原 10 个字段不变，评论数均严格大于 50。
